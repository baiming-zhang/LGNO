...
class NormalizedResidualLGNOAmp2(nn.Module):
    ...
    def __init__(
        ...
        assert variant == "folded", "Use LGNO_unfolded.py for the unfolded baseline."
        ...
        # Shared MLP input for each branch:
        #   q_norm stencil + V_norm center + |psi|^2_norm center
        #   5pt: 5 + 1 + 1 = 7
        #   9pt: 9 + 1 + 1 = 11
        self.weight_input_dim = self.num_points + 2
        ...
        def make_weight_net():
            ...
            layers.append(nn.Linear(in_dim, self.num_points))
            return nn.Sequential(*layers)

        # Folded/main LGNO: Re and Im branches share one weight MLP and one base stencil.
        self.net = make_weight_net()
    ...
    def build_local_feature(self, u):
        ...
        Returns the Re-branch feature. The Im branch uses the same feature layout
        but replaces Re stencil by Im stencil.
        """
...
def parse_args():
    ...
    parser.add_argument("--out_dir", type=str, default="LGNO_12hidden_results")

    parser.add_argument("--hidden", type=int, default=12)
    parser.add_argument("--depth", type=int, default=3)
    parser.add_argument("--stencil", type=str, default="5pt", choices=["5pt", "9pt"])
    parser.add_argument("--variant", type=str, default="folded", choices=["folded"])
