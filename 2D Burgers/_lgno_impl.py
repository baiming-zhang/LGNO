...
base_dir = get_run_dir_from_cli() or os.path.join(
    "LGNO_8hidden_results", f"run_{timestamp}"
)
...
def save_single_field_image(field2d, save_stem, cmap, vmin, vmax):
    ...
    ax.tick_params(axis="both", direction="out", length=3, width=0.8, labelsize=26)
...
def save_all_test_images(f_pred_test, f_test):
    ...
    - normalize using the max value of the truth field
    - use the same color scale for prediction and truth
    - normalize the error separately with a symmetric scale
    """
    ...
    for b in range(B):
        ...
        # ====================================================
        # use the truth field for normalization (important)
        # ====================================================
        ...
        # normalize the error separately
        ...
        save_single_field_image(
            field2d=pred_img, save_stem=pred_name, cmap="RdBu_r", vmin=vmin, vmax=vmax
        )

        save_single_field_image(
            field2d=true_img, save_stem=true_name, cmap="RdBu_r", vmin=vmin, vmax=vmax
        )
        ...
        # ====================================================
        # use a separate colorbar for each sample
        # ====================================================
...
if __name__ == "__main__":
    ...
    if not args.eval_only:
        train_time, loss_history = train(
            model, u_train, f_train, args.lr, args.epochs, args.patience
        )
